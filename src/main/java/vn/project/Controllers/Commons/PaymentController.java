...
import vn.project.Service.CheckoutFacade;
...
public class PaymentController {
	...
	public PaymentController(IUserService userService, ICartService cartService,
			IOrder_ProductService orderproductService, IOrderService orderService) {
		this.userService = userService;
		this.cartService = cartService;
		this.orderproductService = orderproductService;
		this.orderService = orderService;
		this.checkoutFacade = new CheckoutFacade(orderService, orderproductService, cartService);
	}

	@GetMapping("/create_payment") // xu li truyen qua vnpay de thanh toan
	public String payment(@ModelAttribute("totalamount") String totalamount, HttpServletRequest request) {
	...
	public String paymentreturn(HttpSession session, @RequestParam String vnp_ResponseCode) {
		try {
			...
			if ("00".equals(vnp_ResponseCode)) {
				List<Order_Products> listOrder_Products = (List<Order_Products>) session
...
//				}
				...
				checkoutFacade.completeCheckout(user, order, listOrder_Products);
			...
			if (present.isPresent()) {
				...
				for (CartDTO cart : usercart) {
					totalprice = totalprice
							+ (Integer.parseInt(cart.getPrice()) * Integer.parseInt(cart.getQuantity()));
				}
