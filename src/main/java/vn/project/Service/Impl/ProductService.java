...
import vn.project.Service.ProductIterator;
import vn.project.Service.ProductList;
...
//	}
	...
	@Override
	public Page<ProductsDTO> findAllDTO(Pageable pageable) {
		Page<Products> pageProducts = productRepository.findAll(pageable);

		List<ProductsDTO> productDTOs = ConvertProductToProductDTO(pageProducts.getContent());
		...
		return pageProducts.map(product -> {
			...
			dto.setPrice(Long.toString(product.getPrice()));
			dto.setStockquantity(String.valueOf(product.getStockquantity()));

			Brands brand1 = brandRepository.findById(product.getBrandid()).orElse(null);
			dto.setBrand(brand1 != null ? brand1.getBrandname() : "Unknown brand name");

			Categories cate = categoryRepository.findById(product.getCategoryid()).orElse(null);
			dto.setCategory(cate != null ? cate.getCategoryname() : "Unknown category name");

			Suppliers supplier = supplierRepository.findById(product.getSupplierid()).orElse(null);
			dto.setSupplier(supplier != null ? supplier.getSuppliername() : "Unknown supplier name");
	...
	@Override
	public Page<ProductsDTO> findAllSortedByPriceAsc(Pageable pageable) {
		Page<Products> pageProducts = productRepository.findAllByOrderByPriceAsc(pageable);
		return pageProducts.map(this::convertToDTO);
	}

	@Override
	public Page<ProductsDTO> findAllSortedByPriceDesc(Pageable pageable) {
		Page<Products> pageProducts = productRepository.findAllByOrderByPriceDesc(pageable);
		return pageProducts.map(this::convertToDTO);
	}
	...
	private ProductsDTO convertToDTO(Products product) {
		...
		Brands brand1 = brandRepository.findById(product.getBrandid()).orElse(null);
		dto.setBrand(brand1 != null ? brand1.getBrandname() : "Unknown brand");

		Categories cate = categoryRepository.findById(product.getCategoryid()).orElse(null);
		dto.setCategory(cate != null ? cate.getCategoryname() : "Unknown category");

		Suppliers supplier = supplierRepository.findById(product.getSupplierid()).orElse(null);
		dto.setSupplier(supplier != null ? supplier.getSuppliername() : "Unknown supplier");
