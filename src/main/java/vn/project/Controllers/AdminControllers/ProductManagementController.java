	...
	public String updateProduct(@ModelAttribute ProductsDTO product, RedirectAttributes redirectAttributes) {
		try {
			...
			if (product != null) {
				...
				Products editproduct = Products.builder().productid(product.getProductid()).brandid(brandid)
						.categoryid(cateid).description(product.getDescription()).imageurl(product.getImageurl())
						.price(Integer.valueOf(product.getPrice())).productname(product.getProductname())
						.stockquantity(Integer.valueOf(product.getStockquantity())).supplierid(supplierid).build();
